  ...
  function step2a(s, st) {
    ...
    if (fug !== null) {
      ...
      if (base !== undefined) {
        ...
        // ST limit hierarchy: ST OEL → ST DNEL → LT OEL → LT DNEL
        const lim_st_true = oel_st ?? dnel_st;
        const lim_st_for_score = lim_st_true ?? lim_lt;
        const st_used_lt = lim_st_true === null && lim_st_for_score !== null;

        if (lim_lt !== null) rcr_lt = exp_lt / lim_lt;
        // Compute ST RCR using effective limit; only expose for display when a true ST limit exists
        let rcr_st_internal = null;
        if (lim_st_for_score !== null) rcr_st_internal = exp_st / lim_st_for_score;
        if (!st_used_lt) rcr_st = rcr_st_internal;
        ...
        // Short-term inhalation: LT fallback only applies when there is an inhalation hazard
        let s_st;
        if (rcr_st_internal !== null && (!st_used_lt || hasInhalHaz)) { s_st = rcrScore(rcr_st_internal, false); }
        ...
        return {
          ...
          lim_lt, lim_st: lim_st_true, dnel_d,
          st_used_lt: st_used_lt && hasInhalHaz,
          stLtFallbackLabel: st_used_lt && hasInhalHaz && s_st > 1,
